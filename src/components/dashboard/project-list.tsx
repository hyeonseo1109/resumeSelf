...
export function ProjectList({ projects, tier, limit }: ProjectListProps) {
  return (
        ...
        {projects.map((project) => (
          <article key={project.id} className="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2">
              ...
              <button className="inline-flex items-center gap-1.5 rounded-md border border-zinc-200 px-3 py-2 text-sm font-medium text-zinc-700">
                <Copy className="size-4" />
                복제
              </button>
              <button className="ml-auto inline-flex size-9 items-center justify-center rounded-md text-zinc-400 hover:bg-red-50 hover:text-red-600">
